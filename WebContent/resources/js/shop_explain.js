...
function manageInterest(contextPath, obj){
	...
	xhttp.onload = function() {
		if(this.responseText.search(/로그인/igm) != -1){
			location.href = contextPath + "/login?msg="+this.responseText;
			return;
		}
...
function goToBuyPage(path){
	...
	document.forms[1].action = path + "/pageBuy";
	document.forms[1].submit();
}
...
/* 장바구니 이동
*/
function putItemOnCart(path){
	//document.getElementById("pagePd").value = "proCart";
	document.forms[1].action = path + "/addCart";
	document.forms[1].submit();
}