<%@page import="sample.spring.dto.ShopSMSSend"%>
...
<link rel = "stylesheet" href = "resources/css/shop_sms.css">
</head>
		...
		function send(smsChk) {
			...
			if(document.getElementById("cate").value=="password"){
				if(smsChk==sms.value){
					...
					document.forms[0].action += '/frmPwdFine';
					document.forms[0].submit();
				}else{
			...
			if(document.getElementById("cate").value=="id"){
				if(smsChk==sms.value){
					...
					document.forms[0].action += '/frmIdFine';
					document.forms[0].submit();
				}else{
					alert("인증번호가 일치하지 않습니다.");
					vPwd.select();
			...
			if(document.getElementById("cate").value=="phone"){
				if(smsChk==sms.value){
					alert("인증번호 일치 확인. 해당 메뉴로 이동합니다.")
					document.forms[0].action = '<%=contextPath%>'+"/membership";
