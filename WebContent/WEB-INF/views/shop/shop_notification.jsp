...
<head>
    ...
    //경로 설정
		...
		/* String cssConPath = contextPath + "/resource/css/shop_headerContainer.css";
		String cssMainPath = contextPath + "/resource/css/shop_notification.css";
		String cssPagination = contextPath + "/resource/css/pagination.css"; */
...
<%
    ...
    <div id="wrapper">
        ...
        <ul>
            ...
            <li><div class="licc"><a href="<%=contextPath %>/shop_notification_content?notification_No=<%=dtoL.get(idx).getNotification_No()%>"><%=dtoL.get(idx).getNotification_Subject() %></a></div><hr></li>            
        	<%
            }%>
            <div class="buttons">				
				<a class="buts" href="<%=contextPath %>/shop_notification_write">공지사항 작성하기</a>
			</div>
			<% if(totalBlock > 0) { // 블록의 페이지 수가 0보다 큰 경우에만 출력 %>
            <%
            if(nowBlock != 0){ // nowBlock이 0이면 뒤로가기 출력 X
            %>
            	<a class = "select" href = "./shop_notification?page=pageNotification&nowPage=<%=(nowBlock-1)*pagePerBlock%>&nowBlock=<%=nowBlock-1%>">≪</a>
            <%
            }
            %>
        	<%
        	...
        	%>
        		<a class = "select" href = "./shop_notification?page=pageNotification&nowPage=<%=idx1%>&nowBlock=<%=nowBlock%>"><%=idx1+1%></a>
        	<%
